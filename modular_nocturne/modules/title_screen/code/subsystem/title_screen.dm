...
/datum/controller/subsystem/title/Initialize()
	var/dat
	if(!fexists("[global.config.directory]/nocturne/title_html.html"))
		...
		dat = file2text("[global.config.directory]/nocturne/title_html.html")
...
/datum/controller/subsystem/title/proc/update_character_name(mob/dead/new_player/user, name)
	...
	user.client << output(name, "nocturne_title_browser:update_current_character")
...
/proc/add_startup_message(msg, warning)
	...
	for(var/mob/dead/new_player/new_player in GLOB.new_player_list)
		...
		new_player.client << output(msg_html, "nocturne_title_browser:append_terminal_text")
		new_player.client << output(list2params(list(new_timing, SStitle.average_completion_time)), "nocturne_title_browser:update_loading_progress")
